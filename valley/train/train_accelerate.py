...
class SFTMetric:
    ...
    def update(self, logits, labels, loss):
        ...
        with torch.no_grad():
            ...
            self.right += (shift_preds == shift_labels).masked_fill(
                shift_labels.eq(-100), 0).sum().item()
...
def train():
    ...
    accelerator = Accelerator(
        mixed_precision='fp16', log_with="wandb", project_dir=training_args.output_dir)
    ...
    if data_args.video_data_path:
        tokenizer.add_tokens([DEFAULT_VIDEO_FRAME_TOKEN, DEFAULT_VI_START_TOKEN,
                             DEFAULT_VI_END_TOKEN], special_tokens=True)
    ...
    num_training_steps = (len(
        train_dataloader) * training_args.num_train_epochs) // accelerator.gradient_accumulation_steps
    ...
    progress_bar = tqdm(range(len(train_dataloader)*int(training_args.num_train_epochs)),
    ...
    for epoch in range(int(training_args.num_train_epochs)):
        for batch_cnt, inputs in enumerate(train_dataloader):
            try:
                ...
                if accelerator.is_main_process:
                    accelerator.print(
                        f"epoch: {epoch}, cureent step: {batch_cnt}, total step: {len(train_dataloader)}, skip:{accelerator.optimizer_step_was_skipped}, loss:{round(train_loss, 3)}, acc:{round(acc, 3)}, length:{len(inputs.pop('input_ids')[0])}, lr:{lr_scheduler.get_last_lr()[0]}")
                ...
                if global_step % training_args.save_steps == 0:
                    accelerator.save_state(
                        training_args.output_dir+'/checkpoint-'+str(global_step))
